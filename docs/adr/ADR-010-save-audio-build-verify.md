...
## 2026-08-09 补充：音频资源必须同时验证文件、ID 与发布产物

音频文件存在不等于玩家能够听到。`AudioManager.playEffect()` 对未知 ID 会安全忽略，因此资源表与调用方命名不一致会形成静默失效。所有玩法音效必须注册在 `AUDIO_PATHS`，并由 `npm run lint:audio` 扫描源码中的字面量 ID、验证文件路径和 WAV 基本质量。动态 ID（例如 `ending_ + type`）不能只跳过检查，必须在门禁中显式枚举所有合法展开值。

天气音频是区域环境音之上的独立低音量循环层，使用单独的 `weatherEl` 生命周期；切换天气、静音、退出游戏界面和浏览器首次交互解锁时，都必须与主环境音分别处理。不得用天气切换替换区域环境音，否则区域声景会被天气永久覆盖。

发布音效遵循以下规则：

- 战斗、结局、天气、安全屋、探索和仪式使用非语言音效，不依赖人物对白。
- reducer/system 只推送序列化的 `AUDIO_*` effect，实际播放仍由 post-reducer 执行器完成。
- 长循环必须检查首尾接缝；全部新 WAV 必须检查有效头、非静音、零削波。
- 构建版 E2E 必须从 `/audio/` 请求完整发布音效包并验证 RIFF/WAVE 文件头，防止源码存在但 `dist` 漏拷贝。

归档目录（例如被 `.gitignore` 排除的 `scripts/archive/`）不是仓库运行时契约，CI 测试不得断言其中的历史脚本存在。迁移一次性模拟器后，测试应验证当前 `package.json` 命令所指向的维护入口，避免本地归档存在时通过、GitHub 检出后失败。
