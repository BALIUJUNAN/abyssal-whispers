...
## 2026-08-08 补充：分支模块不得反向 import 调度器

从大型 switch 提取分支函数时，调度器可以 import 分支模块，但分支模块不能为了复用一个小工具再反向 import 调度器，否则所有分支会形成 ESM 环。共享工具应移动到双方都依赖的叶子模块（例如 NPC 信任反馈归入 `npcFeedback.js`）。依赖图检查需要忽略注释中的示例 import，避免把文档文本误报为循环。
