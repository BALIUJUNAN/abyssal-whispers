...
## 2026-08-09 补充：结局条件解析失败不得降级为真

数据驱动结局中，未识别表达式若解析成 `always_true`，会把“永远不可达”与“无条件放行”同时隐藏起来。条件引擎必须：

- 支持字符串等值（如 `fear_primary == ocean`），不得只解析数字 `==`。
- 遵循常规布尔优先级：`AND` 高于 `OR`，因此递归解析时先切分低优先级的 `OR`。
- 未知变量和未支持语法默认评估为 false。
- CI 静态门禁枚举全部注册结局，确认每个叶子表达式可解析且每个计数变量已在 `CONDITION_VAR_MAP` 注册。
