...
## 2026-08-10 补充：延迟交互仍属于同一次探索事务

`EXPLORE` 遇到 choice/gamble 时可以暂停直接后果，但玩家完成 `CHOICE_SELECT` / `GAMBLE_CHOICE` 后必须调用同一个 `_postExploreProcessing` 收尾。目标检查、线索链、结论、事件链日志、怪物显现和首次探索标记都属于原探索事务，不能只在“无弹窗事件”路径执行，也不能等下一次探索偶然补算。每个延迟分支必须有独立回归测试证明收尾恰好执行一次。

终局日边界也属于生命周期协议：第 28 天是最后一个可行动日，未达成其他结局时只能在该日结束、推进到第 29 天后触发时间耗尽结局。日开启流程若已经由 `checkEnding` 选出具体结局，不得再用通用超时对象覆盖它。
