 ...
 * Run: node tests/test_game_data_protocol.mjs
 */
...
// Load data
const base = JSON.parse(fs.readFileSync(join(SRC, 'data', 'game_base.json'), 'utf8'));
let ch2plus = {};
let meta = {};
try {
  ch2plus = JSON.parse(fs.readFileSync(join(SRC, 'data', 'game_ch2plus.json'), 'utf8'));
} catch (e) {}
try {
  meta = JSON.parse(fs.readFileSync(join(SRC, 'data', 'game_meta.json'), 'utf8'));
} catch (e) {}
...
// Static imports from src/ (ESM)
import { resolveNpcId, NPC_REGISTRY, migrateNpcKeys, getNpcName } from '../src/data/registry/npcRegistry.js';
import { resolveItemId, ITEM_REGISTRY, normalizeItemRef, migrateInventory } from '../src/data/registry/itemRegistry.js';
import { createRegistryHelpers } from '../src/data/registry/registryUtils.js';

// Validator (CJS module — Node.js ESM interop handles module.exports named exports)
import { validateGameData } from '../scripts/validators/validateGameData.cjs';
  ...
  for (const end of allEndings) {
    ...
    for (const c of conds) {
      ...
      for (const ch of c) {
        ...
        if (ch === ')') depth--;
        if (depth < 0) {
          bad.push(end.id + ': unbalanced "' + c + '"');
          break;
        }
      }
      if (depth !== 0 && !bad.some((b) => b.includes(end.id)))
        bad.push(end.id + ': unbalanced "' + c + '"');
    }
  }
...
test('npcRegistry uses registryUtils when available', function () {
  // npcRegistry.js already imports createRegistryHelpers at top level
  assert.strictEqual(resolveNpcId('玛莎·格雷'), 'martha_grey');
  assert.strictEqual(getNpcName('martha_grey'), '玛莎·格雷');
  const migrated = migrateNpcKeys({ '玛莎·格雷': 3, '希尔达·莫里斯': 5 });
