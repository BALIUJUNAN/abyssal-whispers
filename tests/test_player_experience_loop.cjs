/**
 * tests/test_player_experience_loop.cjs
 * Full player experience chain integration test.
 * Run: node tests/test_player_experience_loop.cjs
 */
...
let GD = {};
try { GD = JSON.parse(require('fs').readFileSync(path.join(ROOT, 'game_base.json'), 'utf8')); } catch (e) {}
const ctx = { GD };

const { buildDeathSummary } = require(path.join(ROOT, 'src/systems/deathSummary.js'));
const { computeReincarnationDiff } = require(path.join(ROOT, 'src/systems/reincarnationDiff.js'));
const { getGuideStep } = require(path.join(ROOT, 'src/systems/firstRunGuide.js'));
...
test('P8-1: DEFAULT_SETTINGS has accessibility fields', () => {
  for (const k of ['fontSize', 'flickerControl', 'reduceMotion', 'visualPollution', 'screenShake', 'textCorruption', 'vignetteIntensity', 'showGuideHints']) {
