...
const SAN_SLICES = [
  ...
  // Explore domain: SAN loss lives in moveSystem.js and explorePipeline.js
  'src/systems/explore/moveSystem.js',
...
test('S8-1: all SAN loss slices have death resolution in same file or caller', () => {
  ...
  const slicesViaMainReducer = [
    'src/reducers/slices/darkSlice.js',
    'src/reducers/slices/npcSlice.js',
    'src/reducers/slices/exploreSlice.js', // thin dispatcher → explorePipeline handles death resolution
    'src/reducers/objectiveReducer.js',
    'src/reducers/effectReducer.js',
  ];
...
test('S9-4: firstLoopBalance functions used in correct locations', () => {
  const ep = fs.readFileSync(path.join(ROOT, 'src/systems/explore/explorePipeline.js'), 'utf8');
  const ess = fs.readFileSync(path.join(ROOT, 'src/systems/explore/eventSelectionSystem.js'), 'utf8');
  assert.ok(ep.includes("from '../../systems/firstLoopBalance.js'"), 'explorePipeline should import firstLoopBalance');
  assert.ok(ep.includes('adjustSanLossForLoop23'), 'explorePipeline should call adjustSanLossForLoop23');
  assert.ok(ess.includes('shouldBlockLethalEvent'), 'eventSelectionSystem should call shouldBlockLethalEvent');
});

test('S9-5: all 8 system modules exist in src/systems/', () => {
  const modules = [
...
test('S9-10: textVariants is imported in explore domain files', () => {
  const move = fs.readFileSync(path.join(ROOT, 'src/systems/explore/moveSystem.js'), 'utf8');
  const trp = fs.readFileSync(path.join(ROOT, 'src/systems/explore/textRenderingPipeline.js'), 'utf8');
  assert.ok(move.includes("from '../../systems/textVariants.js'") || trp.includes("from '../../systems/textVariants.js'"), 'explore domain should import textVariants');
  assert.ok(move.includes('applyMythosAliases') || trp.includes('applyMythosAliases'), 'explore domain should call applyMythosAliases');
});

test('S9-11: no orphaned settings in miscReducer DEFAULT_SETTINGS', () => {
  const content = fs.readFileSync(path.join(ROOT, 'src/reducers/miscReducer.js'), 'utf8');
  // These were orphaned settings with no game logic behind them
  const dead = ['fontSize', 'lineHeight', 'fontFamily', 'highContrast', 'screenShake',
    'textCorruption', 'vignetteIntensity', 'masterVolume', 'musicVolume',
    'sfxVolume', 'voiceVolume', 'muteAll', 'autoSave', 'skipSeenText', 'confirmActions'];
  for (const key of dead) {
    assert.ok(!content.includes(key + ':'), 'orphaned setting ' + key + ' should not be in DEFAULT_SETTINGS');
  }
});
...
test('S10-4: seenEventTexts persists across loops', () => {
  ...
  const trp = fs.readFileSync(path.join(ROOT, 'src/systems/explore/textRenderingPipeline.js'), 'utf8');
  assert.ok(trp.includes('seenEventTexts'), 'textRenderingPipeline should use seenEventTexts');
