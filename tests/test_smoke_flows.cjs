...
function test(name, fn) {
  try { fn(); passed++; global.console.log('  PASS: ' + name); }
  catch (e) { failed++; global.console.log('  FAIL: ' + name + ' -> ' + (e.message || e).split('\n')[0]); }
}
...
function makeState(overrides) {
  return Object.assign({
    screen: 'title', san: 60, maxSan: 99, hp: 11, maxHp: 11,
    day: 1, loopCount: 0, pollution: 0, food: 3, maxFood: 5, money: 5,
    ap: 6, maxAp: 6, currentArea: 'town_center', visitedAreas: ['town_center'],
    triggeredEvents: [], npcTrust: {}, npcStates: {}, inventory: [], clues: [],
    skills: {}, objectives: [], narrative: [], eventLog: [],
    stats: { STR:50, CON:55, DEX:55, POW:60, INT:65, SIZ:60, EDU:70, APP:50 },
    behaviorTracking: {}, stats_run: {}, longTermEffects: [],
    difficulty: 'normal', weather: '阴天', sealState: 'intact',
    currentSafehouse: 'main', safehouseCorruption: 0, tutorialSeen: {},
    _dayActions: [], _actionHistory: [], _effects: [],
  }, overrides);
}
...
test('smoke: MOVE -> area change', function() {
  var s = makeState({ screen: 'game', ap: 3, currentArea: 'town_center',
    visitedAreas: ['town_center'], clues: [{id:'c1',name:'test'}] });
  var next = gr(s, { type: 'MOVE', areaId: 'harbor_district', cost: 1 });
  if (next.currentArea !== 'harbor_district') throw new Error('Expected harbor_district, got ' + next.currentArea);
});
