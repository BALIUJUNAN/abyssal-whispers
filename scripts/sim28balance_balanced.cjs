...
// 恐惧画像系统 - 最终平衡版本
const FEARS = {
  thalassophobia: { name: '深海恐惧', difficulty_level: 1 },   // 最简单
  claustrophobia: { name: '幽闭恐惧', difficulty_level: 5 },   // 最难
  nyctophobia: { name: '黑暗恐惧', difficulty_level: 3 },       // 中等
  acrophobia: { name: '高空恐惧', difficulty_level: 2 },       // 简单
  insectophobia: { name: '虫类恐惧', difficulty_level: 4 }     // 难
};
...
// 根据难度等级计算修正系数 - 增强版本
function getDifficultyModifier(level) {
...
function simRun(idx,fearKey,profKey,pKey){
  const fearProfile = FEARS[fearKey] || { difficulty_level: 3 };
  const difficultyLevel = fearProfile.difficulty_level || 3;
  const fearSanMod = getDifficultyModifier(difficultyLevel);
  const fearHpMod = getDifficultyModifier(difficultyLevel);
  const fearClueBonus = 1.0 + (3 - difficultyLevel) * 0.1;  // 难度越低，线索越多
