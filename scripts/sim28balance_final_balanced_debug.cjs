...
const FEARS = {
  thalassophobia: { name: '深海恐惧', san_mod: 1.3, hp_mod: 0.7 },   // SAN+30%, HP-30%
  claustrophobia: { name: '幽闭恐惧', san_mod: 0.6, hp_mod: 1.4 },   // SAN-40%, HP+40%
  nyctophobia: { name: '黑暗恐惧', san_mod: 0.7, hp_mod: 1.3 },       // SAN-30%, HP+30%
  acrophobia: { name: '高空恐惧', san_mod: 1.2, hp_mod: 0.8 },       // SAN+20%, HP-20%
...
function simRun(idx,fearKey,profKey,pKey){
  ...
  if (idx < 3) console.log('DEBUG: fearKey=' + fearKey + ', san_mod=' + fear.san_mod + ', hp_mod=' + fear.hp_mod);
