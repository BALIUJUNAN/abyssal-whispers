...
const DIFFICULTY_CONFIG = {
  ...
  normal: {
    ...
    san_protection: { day_1_3: 0.5, day_4_7: 0.7, day_8_14: 0.85, day_15_21: 0.95, day_22_28: 1.0 },
    hp_protection: { day_1_3: 0.5, day_4_7: 0.7, day_8_14: 0.85, day_15_21: 0.95, day_22_28: 1.0 },
    max_san_per_action: 3, max_san_per_day: 8, max_hp_per_action: 2, max_hp_per_day: 4,
    explore_san_chance: 0.12, explore_danger_mult: 0.04, monster_hp_chance: 0.08, monster_hp_max: 3,
    rest_san_recovery: 2, talk_san_recovery: { chance: 0.35, min: 1, max: 3 },
    day_recovery: { interval: 3, san: 1, hp: 1 }, safe_zone_restriction: 5,
  },
  hard: {
...
const FEARS={thalassophobia:{harbor_district:1.1},claustrophobia:{catacombs_entrance:1.1,deep_catacombs:1.15},nyctophobia:{whispering_forest:1.1},acrophobia:{lighthouse:1.15},insectophobia:{whispering_forest:1.1,forbidden_grove:1.15}};
    ...
    while(s.ap>0&&s.hp>0&&s.san>0){
      ...
      switch(act.t){
        case"mv":
          ...
          if(ar&&ar.danger>=3&&sr()<.08*ar.danger){let loss=ri(1,Math.round(ar.danger*fm));loss=applyProtection(loss,s.day,CFG.san_protection,CFG.max_san_per_action);if(s.sanLostToday+loss<=CFG.max_san_per_day){s.san=cl(s.san-loss,0,s.mSan);s.sanLostToday+=loss;}}
          break;
        case"ex":{
          s.ap--;const ar2=AREAS[s.area];const d=ar2?ar2.danger:1;const sm=sealState(s.day).global_modifier?.san_loss_multiplier||1;
          if(sr()<CFG.explore_san_chance+d*CFG.explore_danger_mult){let loss=Math.round(ri(1,2+d)*sm);loss=applyProtection(loss,s.day,CFG.san_protection,CFG.max_san_per_action);if(s.sanLostToday+loss<=CFG.max_san_per_day){s.san=cl(s.san-loss,0,s.mSan);s.sanLostToday+=loss;}}
          if(sr()<.25)s.clues.push("c"+s.clues.length);
          if(d>=4&&sr()<CFG.monster_hp_chance*d*(s.day>14?1.5:1)){let hpLoss=ri(1,CFG.monster_hp_max);hpLoss=applyProtection(hpLoss,day,CFG.hp_protection,CFG.max_hp_per_action);if(s.hpLostToday+hpLoss<=CFG.max_hp_per_day){s.hp=Math.max(0,s.hp-hpLoss);s.hpLostToday+=hpLoss;}}
          break;}
