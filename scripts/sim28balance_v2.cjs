...
const MAX_SAN_LOSS_PER_ACTION = 3;
const MAX_SAN_LOSS_PER_DAY = 8;
...
function simRun(idx,fearKey,profKey,pKey){
  ...
  for(let day=1;day<=28;day++){
    ...
    while(s.ap>0&&s.hp>0&&s.san>0){
      ...
      switch(act.t){
        case"mv":
          ...
          if(ar&&ar.danger>=3&&sr()<.08*ar.danger){let loss=ri(1,Math.round(ar.danger*fm));loss=applyDayProtection(loss,s.day);loss=Math.min(loss,MAX_SAN_LOSS_PER_ACTION);s.san=cl(s.san-loss,0,s.mSan);}
          break;
        case"ex":{
          s.ap--;const ar2=AREAS[s.area];const d=ar2?ar2.danger:1;const sm=sealState(s.day).global_modifier?.san_loss_multiplier||1;
          if(sr()<.15+d*.05){let loss=Math.round(ri(1,2+d)*sm);loss=applyDayProtection(loss,s.day);loss=Math.min(loss,MAX_SAN_LOSS_PER_ACTION);s.san=cl(s.san-loss,0,s.mSan);}
          if(sr()<.25)s.clues.push("c"+s.clues.length);
          if(d>=4&&sr()<.08*d*(s.day>14?1.5:1))s.hp=Math.max(0,s.hp-ri(1,3));
          break;}
        case"tk":{s.ap--;const tr=s.nt[act.n]||0;if(sr()<.4&&tr<5)s.nt[act.n]=tr+1;if(tr>=2&&sr()<.35)s.san=cl(s.san+ri(1,3),0,s.mSan);break;}
    ...
    else{s.starve=0;s.hp=cl(s.hp+1,0,s.mHp);s.san=cl(s.san+2,0,s.mSan);}
...
console.log("=== 28-Day Balance Simulation v2 (With Protection) ===");
console.log("Runs:",RUNS,"Seed:",IS,"Fear:",FEAR);
console.log("Protection: Day 1-3: 70% reduction, Day 4-7: 50%, Day 8-14: 25%");
console.log("Limits: Max 3 SAN/action, Max 8 SAN/day");
const t0=Date.now();
const results=[];
