...
function simRun(idx,fearKey,profKey,pKey){
  ...
  const difficultyMod = fearProfile.difficulty_modifier || 1.0;  // 全局难度修正
  console.log('DEBUG: fearKey=' + fearKey + ', difficultyMod=' + difficultyMod);
...
if(ar&&ar.danger>=3&&sr()<.08*ar.danger){
  let loss=ri(1,Math.round(ar.danger*fm*areaFearMod*difficultyMod));
...
if(sr()<CFG.explore_san_chance+d*CFG.explore_danger_mult){
  let loss=Math.round(ri(1,2+d)*sm*difficultyMod);
  loss=applyProtection(loss,s.day,CFG.san_protection,CFG.max_san_per_action);
  if(s.sanLostToday+loss<=CFG.max_san_per_day){s.san=cl(s.san-loss,0,s.mSan);s.sanLostToday+=loss;}
}
          if(sr()<.25/difficultyMod)s.clues.push("c"+s.clues.length);
          if(d>=4&&sr()<CFG.monster_hp_chance*d*(s.day>14?1.5:1)){let hpLoss=Math.round(ri(1,CFG.monster_hp_max)*difficultyMod);hpLoss=applyProtection(hpLoss,day,CFG.hp_protection,CFG.max_hp_per_action);if(s.hpLostToday+hpLoss<=CFG.max_hp_per_day){s.hp=Math.max(0,s.hp-hpLoss);s.hpLostToday+=hpLoss;}}
          break;}
