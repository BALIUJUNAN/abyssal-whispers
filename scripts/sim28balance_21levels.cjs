...
// 21级难度系统
const DIFFICULTY_LEVELS = JSON.parse(fs.readFileSync(path.join(ROOT, 'src/config/difficultyLevels.json'), 'utf8'));
const LEVEL = parseInt(ga('level', '1'), 10);
const CFG = DIFFICULTY_LEVELS[LEVEL] || DIFFICULTY_LEVELS[1];



// 恐惧画像系统 - 每个画像都有独特优势和劣势
...
// 恐惧画像系统 - 简单平衡版本
// difficulty_modifier: < 1.0 = 更容易, > 1.0 = 更难
...
const FEARS = {
  thalassophobia: {
    name: '深海恐惧',
    difficulty_modifier: 1.0,  // 基准难度
    affected_areas: { harbor_district: 1.3, lighthouse: 1.2 }  // 在码头和灯塔更难
  },
  claustrophobia: {
    name: '幽闭恐惧',
    difficulty_modifier: 0.85,  // 整体更容易（补偿高危险区域）
    affected_areas: { catacombs_entrance: 1.2, deep_catacombs: 1.3 }  // 在墓穴更难
  },
  nyctophobia: {
    name: '黑暗恐惧',
    difficulty_modifier: 0.90,  // 整体更容易
    affected_areas: { whispering_forest: 1.2, forbidden_grove: 1.3 }  // 在森林更难
  },
  acrophobia: {
    name: '高空恐惧',
    difficulty_modifier: 1.0,  // 基准难度
    affected_areas: { lighthouse: 1.3, voxchester_manor: 1.2 }  // 在灯塔和庄园更难
  },
  insectophobia: {
    name: '虫类恐惧',
    difficulty_modifier: 0.95,  // 整体略容易
    affected_areas: { whispering_forest: 1.2, forbidden_grove: 1.3 }  // 在森林更难
  }
};
...
function simRun(idx,fearKey,profKey,pKey){
  const fearProfile = FEARS[fearKey] || {};
  const fearDifficulty = fearProfile.difficulty_modifier || 1.0;
  // 全局SAN损失修正：difficulty_modifier < 1.0 = 损失减少，> 1.0 = 损失增加
  const globalSanModifier = fearDifficulty;
  const globalHpModifier = fearDifficulty;
;
    ...
    while(s.ap>0&&s.hp>0&&s.san>0){
      ...
      switch(act.t){
        case"mv":
          ...
          // 应用恐惧画像的区域影响
const areaFearMod = fearProfile.affected_areas?.[act.d] || 1.0;
if(ar&&ar.danger>=3&&sr()<.08*ar.danger){
  let loss=ri(1,Math.round(ar.danger*fm*areaFearMod*globalSanModifier));
  loss=applyProtection(loss,s.day,CFG.san_protection,CFG.max_san_per_action);
  if(s.sanLostToday+loss<=CFG.max_san_per_day){s.san=cl(s.san-loss,0,s.mSan);s.sanLostToday+=loss;}
}
          break;
        case"ex":{
          s.ap--;const ar2=AREAS[s.area];const d=ar2?ar2.danger:1;const sm=sealState(s.day).global_modifier?.san_loss_multiplier||1;
          // 应用恐惧画像的全局SAN修正
if(sr()<CFG.explore_san_chance+d*CFG.explore_danger_mult){
  let loss=Math.round(ri(1,2+d)*sm*globalSanModifier);
  loss=applyProtection(loss,s.day,CFG.san_protection,CFG.max_san_per_action);
  if(s.sanLostToday+loss<=CFG.max_san_per_day){s.san=cl(s.san-loss,0,s.mSan);s.sanLostToday+=loss;}
}
          if(sr()<.25/difficultyMod)s.clues.push("c"+s.clues.length);
          if(d>=4&&sr()<CFG.monster_hp_chance*d*(s.day>14?1.5:1)){let hpLoss=Math.round(ri(1,CFG.monster_hp_max)*globalHpModifier);hpLoss=applyProtection(hpLoss,day,CFG.hp_protection,CFG.max_hp_per_action);if(s.hpLostToday+hpLoss<=CFG.max_hp_per_day){s.hp=Math.max(0,s.hp-hpLoss);s.hpLostToday+=hpLoss;}}
          break;}
        case"tk":{s.ap--;const tr=s.nt[act.n]||0;if(sr()<.4&&tr<5)s.nt[act.n]=tr+1;// 应用恐惧画像的社交优势

if(tr>=2&&sr()<CFG.talk_san_recovery.chance){
  let recovery = ri(CFG.talk_san_recovery.min, CFG.talk_san_recovery.max);
  s.san=cl(s.san+recovery,0,s.mSan);
}break;}
  ...
  s.san=cl(s.san+CFG.rest_san_recovery,0,s.mSan);
}
...
console.log("=== 28-Day Balance Simulation (Level " + LEVEL + ": " + CFG.name + ") ===");
