...
// 恐惧画像系统 - 平衡版本
const FEARS = {
  thalassophobia: { name: '深海恐惧', san_modifier: 1.0, hp_modifier: 1.0, clue_bonus: 1.0 },
  claustrophobia: { name: '幽闭恐惧', san_modifier: 0.9, hp_modifier: 1.1, clue_bonus: 1.1 },
  nyctophobia: { name: '黑暗恐惧', san_modifier: 0.95, hp_modifier: 1.05, clue_bonus: 1.05 },
  acrophobia: { name: '高空恐惧', san_modifier: 1.05, hp_modifier: 0.95, clue_bonus: 0.95 },
  insectophobia: { name: '虫类恐惧', san_modifier: 1.0, hp_modifier: 1.0, clue_bonus: 1.0 }
};
...
function simRun(idx,fearKey,profKey,pKey){
  const fearProfile = FEARS[fearKey] || { san_modifier: 1.0, hp_modifier: 1.0, clue_bonus: 1.0 };
  const fearSanMod = fearProfile.san_modifier || 1.0;
  const fearHpMod = fearProfile.hp_modifier || 1.0;
  const fearClueBonus = fearProfile.clue_bonus || 1.0;
  if (idx < 3) console.log('DEBUG: fearKey=' + fearKey + ', fearSanMod=' + fearSanMod + ', fearHpMod=' + fearHpMod);
