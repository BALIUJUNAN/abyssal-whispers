...
for (const [name, refs] of Object.entries(npcRefs)) {
  console.log(name + ': events=' + refs.events.length + ' effects=' + refs.effects.length + ' endings=' + refs.endings.length);
}
...
for (const evt of allEvents) {
  ...
  for (const c of nc) {
    const n = c.name || c.npc;
    if (n && npcNames.includes(n)) { nameAsKeyCount++; }
  }
  const trigger = evt.trigger || {};
  const cond = typeof trigger.condition === 'string' ? trigger.condition : '';
  for (const name of npcNames) {
    if (cond.includes(name + 'trust') || cond.includes(name + 'Trust')) { nameAsKeyCount++; }
  }
}
...
for (const evt of allEvents) {
  ...
  for (const m of matches) { if (!clueRefs[m]) clueRefs[m] = 0; clueRefs[m]++; }
}
...
var baselines = {};
try { baselines = JSON.parse(fs.readFileSync(path.join(SRC, 'data', 'validators', 'baselines.json'), 'utf8')); } catch(e) {}
...
for (var _ei = 0; _ei < allEvents.length; _ei++) {
  var _items = allEvents[_ei].effects && allEvents[_ei].effects.items ? allEvents[_ei].effects.items : [];
...
console.log('  Chinese NPC refs:   ' + nameAsKeyCount + ' / baseline ' + npcBase + (npcDelta > 0 ? ' UP' + npcDelta + ' NEW DEBT' : npcDelta < 0 ? ' DOWN' + Math.abs(npcDelta) : ' (at baseline)'));
console.log('  Chinese item refs:  ' + itemCount + ' / baseline ' + itemBase + (itemDelta > 0 ? ' UP' + itemDelta + ' NEW DEBT' : itemDelta < 0 ? ' DOWN' + Math.abs(itemDelta) : ' (at baseline)'));
