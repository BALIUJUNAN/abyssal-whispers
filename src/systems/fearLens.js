...
function isEligibleForFearLens(event, tuning) {
  ...
  const eligibleTypes = [
    ...
    'meta', 'silent', 'prologue', 'area_deep',
    'npc_cross', 'loop_locked',
    '超自然遭遇', '环境异常', '线索', 'NPC事件',
    'NPC互动', '调查', '轻微异常', '神秘事件'
  ];
  const eligibleSubtypes = [
    'tutorial_san', 'tutorial_humanity', 'tutorial_clue',
    'area_explore', 'npc_interaction', 'resource_pressure',
    'area_deep', 'resource_pressure', 'loop_locked', 'npc_cross'
  ];
  ...
  if (evtType !== 'prologue' && tags.length > 0) {
    const hasMatch = searchable.some(s =>
      primaryTags.some(pt => s.toLowerCase().includes(pt))
    );
