...
function createErrorTracker() {
  ...
  return {
    ...
    record(action, state) {
      ...
      if (_steps.length > CONFIG.MAX_STEPS) {
        ...
        for (let i = _steps.length - 1; i >= 0; i--) {
          if (_steps[i].isSnapshot) { lastSnapIdx = i; break; }
        }
        const keepFrom = lastSnapIdx > 0 ? lastSnapIdx : _steps.length - Math.floor(CONFIG.MAX_STEPS / 2);
    ...
    exportReport(error, errorInfo) {
      const now = new Date();
      const duration = _sessionStart ? ((now - new Date(_sessionStart)) / 1000).toFixed(1) + 's' : 'N/A';
    ...
    renderText(report) {
      ...
      lines.push('╠════════════════════════════════════════════════════════╣');
      lines.push('');
      lines.push(`  ⏰  ${report.meta.generatedAt}  |  会话时长: ${report.meta.duration}  |  总操作数: ${report.meta.totalSteps}`);
...
function simplifyAction(action) {
  ...
  if (Object.keys(action).length > 5) {
    const extraKeys = Object.keys(action).filter(k => 
      !['type','areaId','choiceId','slotId','targetArea','difficulty','archetype','key'].includes(k)
    );
...
function generateSummary(error, steps) {
  ...
  if (error.message.includes('Cannot access')) summaries.push('初始化顺序错误：某个变量在被使用前尚未定义（TDZ问题）');
  ...
  if (error.message.includes('Unexpected token')) summaries.push('语法错误：代码编译产物可能有损坏');
  
  // 操作模式分析
  const types = steps.map(s => s.type);
  const lastType = types[types.length - 1];
  
  if (lastType === 'BEGIN_ADVENTURE') summaries.push('错误发生在"进入主游戏"阶段，可能是初始数据构建有问题');
  if (lastType === 'START_GAME') summaries.push('错误发生在"新游戏"阶段（前传初始化），检查 prologueReducer');
  if (lastType === 'COMPLETE_PROLOGUE') summaries.push('错误发生在前传完成阶段，检查 fearProfile/calculateFearTuning');
  if (lastType === 'EXPLORE' || lastType === 'SELECT_EVENT') summaries.push('错误发生在探索事件触发环节，检查 eventReducer/selectEventV2');
  ...
  if (lastType === 'TALK_NPC') summaries.push('错误发生在 NPC 交互阶段，检查 npcReducer/trust gates');
