...
export function handleDarkAction(s, action, c, ctx) {
  switch (action.type) {
    ...
    case 'SELF_SACRIFICE': {
      ...
      c.narr(
        'system',
        '你闭上眼，放弃了某种无法命名但你知道一直在那里的东西。然后——力量来了。冰冷，安静，确凿。HP -' +
          hpLoss +
          '，SAN上限永久 -5，克苏鲁神话 +3',
        { isSpecial: true }
      );
    ...
    case 'BREAK_SEAL': {
      ...
      if (['deep_catacombs', 'ruins_of_yith'].includes(s.currentArea))
      ...
      c.narr(
        'system',
        '封印表面出现了一道裂痕。光从裂缝中漏出来。不是自然的光——是某种粘稠的、缓慢流动的光。你感到整个世界晃了一下。SAN -' +
          sanLoss,
        { isSpecial: true }
      );
      return s;
