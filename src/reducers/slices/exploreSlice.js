  ...
  switch (effectType) {
    ...
    case 'npc_permanent_disappear':
      ...
      if (aliveNpcs.length > 0) {
        ...
        setNpcState(state,victim[0],{...getNpcState(state,victim[0]),dead:true,disappearance:"meta_vanish"});
  ...
  if(GD._extendedEventsLoaded){
    ...
    if(candidates.length>0){
      if(s.fearTuning&&s.fearTuning.primary){
        const fearScored=candidates.map(function(ev){return{evt:ev,weight:getEventWeight(ev,s.currentArea,s,ctx)*getFearEventWeightModifier(ev,s)};}).filter(function(x){return x.weight>0;});
        if(fearScored.length>0){
          const totalW=fearScored.reduce(function(a,b){return a+b.weight;},0);
          let roll=Math.random()*totalW;
          for(const item of fearScored){roll-=item.weight;if(roll<=0){evt=item.evt;break;}}
    ...
    if(!evt){
      ...
      if(omen){evt=omen;commitSelectedEvent(omen,s);alreadyCommitted=true;}
      else{
        ...
        if(shouldTriggerMissing600(s,extEvts)&&Math.random()<GAME_BALANCE.MISSING_600_CHANCE){evt=createMissing600Event(s);commitSelectedEvent(evt,s);alreadyCommitted=true;}
      }
    }
...
function _postExploreProcessing(evt, s, c, GD) {
  ...
  for(const ch of chains){
    ...
    if(idx>=0){
      const progress=seq.filter(function(eid){return s.triggeredEvents.includes(eid);}).length;
      if(idx<seq.length-1) c.narr('system','【事件链：'+ch.name+'】进度 '+progress+'/'+seq.length,{isSpecial:true});
    }
  }
  ...
  for(const conc of newConclusions){
    ...
    conc.evidence.forEach(function(e){c.narr('system','  · '+e);});
    conc.unlocks.forEach(function(u){if(!hasClueId(s.clues,u)){const _rn=resolveClueName(u);s.clues.push(_rn&&_rn!==u?{id:u,name:_rn}:u);}});
  }
  // False interpretations
  const falseInts=checkFalseInterpretations(s,ctx);
  for(const fi of falseInts) c.narr('system','【注意】你隐约觉得"'+fi.interpretation+'"这个想法不太对劲。'+(fi.consequence||''),{isSpecial:true});
...
function handleExploreAction(s, action, c, ctx) {
  ...
  case 'MOVE':{
    ...
    if(target==='harbor_district'){c.bt.harbor_visits=(c.bt.harbor_visits||0)+1;c.effects.push({type:'AUDIO_PLAY',id:'harbor_water_omen'});}
    if(target==='lighthouse')c.effects.push({type:'AUDIO_PLAY',id:'lighthouse_lens_crack'});
    if(target==='catacombs_entrance'||target==='deep_catacombs')c.effects.push({type:'AUDIO_PLAY',id:'catacombs_stone'});
    ...
    if(sceneKey&&s.san<GAME_BALANCE.SAN_SCENE_VARIANT_GATE&&Math.random()<GAME_BALANCE.SAN_SCENE_VARIANT_CHANCE){
  ...
  case 'EXPLORE':{
      ...
      for(const ch of chains){
        for(const eid of ch.sequence){
          ...
          if(fe&&!s.triggeredEvents.includes(eid)&&checkTrigger(fe,s)){
            ...
            c.narr('event',feText,{eventTitle:fe.name,eventType:fe.type||fe.event_classification,imageSrc:getEventImage(fe.id)||getAreaSceneImage(s.currentArea,{...c.view,visits:(s.visitedAreas||[]).filter(a=>a===s.currentArea).length}),imageAlt:fe.name});
            return s;
          }
        }
      }
    ...
    c.narr('event',evtText,{eventTitle:evt.name,eventType:evt.type||evt.event_classification,imageSrc:getEventImage(evt.id)||getAreaSceneImage(s.currentArea,{...c.view,visits:(s.visitedAreas||[]).filter(a=>a===s.currentArea).length}),imageAlt:evt.name,_ugcAuthor:evt._ugcAuthor||null});
    if(evt.effects&&evt.effects._meta_effect) applyMetaEffect(evt.effects._meta_effect,s,evt,c);
    // Choices / gamble early exits
    if(evt.choices&&evt.choices.length>0){applyLegacyEffects(s,evt.effects);s.pendingChoice={evt,choices:evt.choices};return s;}
    const gambleOpts=getGambleOptions(evt,s,ctx);
    if(gambleOpts){s.pendingGamble={evt,options:gambleOpts,apSpent:2};c.narr('system','你感到某种冲动——是就此收手，还是更深入地探究？',{isSpecial:true});return s;}
    // Anchor + SAN damage + skill check + madness + death
    const anchorResult=processNormalAnchorEvent(evt,s);
    if(anchorResult.sanGain>0){s.san=clamp(s.san+anchorResult.sanGain,0,s.maxSan);c.narr('san-recovery',anchorResult.text);}
    ...
    if(sanDmg>0){
      ...
      if(isChapter1&&sanDmg>ch1Cap&&!isMidnight){sanDmg=ch1Cap;c.narr('system','（你的直觉告诉你现在不应该深入探究。也许深夜再来会不同。）');}
      ...
      if(sanDmg>0){
        if(evt.skill_check){
          ...
          if(check.success){c.effects.push({type:'AUDIO_SKILL',id:'success'});sanDmg=Math.max(1,Math.round(sanDmg*0.5));c.narr('system','【技能检定：'+check.skillName+'】掷骰 '+check.roll+' / 技能'+check.playerSkill+' —— 成功！SAN损失减半。');s.stats_run.checks_passed++;}
          else{c.effects.push({type:'AUDIO_SKILL',id:check.isCritFail?'critical_fail':'fail'});c.narr('system','【技能检定：'+check.skillName+'】掷骰 '+check.roll+' / 技能'+check.playerSkill+' —— 失败！');s.stats_run.checks_failed++;}
        }
        s.san=clamp(s.san-sanDmg,0,s.maxSan);c.narr('system','SAN -'+sanDmg,{isEffect:true});
        ...
        if(sanDmg>=GAME_BALANCE.MEMORY_TRACK_THRESHOLD){addRunMemory(s,'在'+(s.currentArea||'某处')+'遭遇了什么——SAN -'+sanDmg,'san_loss');c.effects.push({type:'AUDIO_SAN_LOSS',amount:sanDmg});}
      }
    }
    applyLegacyEffects(s,evt.effects);
    if(sanDmg>=GAME_BALANCE.MADNESS_TRIGGER){const mad=rollMadness(ctx);s.madnessActive=mad;c.effects.push({type:'INCREMENT_STAT',key:'madness_count'});c.narr('madness','【临时疯狂：'+mad.name+'】'+mad.description,{madness:mad});addRunMemory(s,'经历了临时疯狂——'+mad.name,'madness');c.effects.push({type:'AUDIO_PLAY',id:'madness'},{type:'AUDIO_PLAY',id:'madness_loop'});}
    {const deathCtx=resolveDeath(s,evt,null);if(deathCtx)applyDeathResolution(s,deathCtx,c.narr);}
  ...
  case 'DO_SKILL_CHECK':{
    ...
    if(result.success){
      ...
      if(Math.random()<GAME_BALANCE.SKILL_IMPROVE_CHANCE)s.skills[result.skillName]=(s.skills[result.skillName]||0)+rand(1,3);
    }else{
