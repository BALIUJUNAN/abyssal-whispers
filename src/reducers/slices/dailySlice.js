...
export function _processFoodAndStarvation(s, c, ctx) {
  ...
  if(s.food<=0){
    ...
    if(sd===1){s.san=clamp(s.san-1,0,s.maxSan);c.narr('system','你腹中空空。胃部的抽搐让你难以集中注意力。',{isSpecial:true});}
  ...
  if(s.hp<=0||s.san<=0){
    ...
    applyDeathResolution(s,{mode:s.hp<=0?'hp':'san',type:deathType,area:s.currentArea,day:s.day,loop:s.loopCount,sourceEventId:null,sourceEventName:'饥饿致死',finalText:deathText,residueFlag:'death_echo_starvation'},c.narr);
    return true;
  }
  return false;
}
...
export function _processSafehouseAndWorldDecay(s, c, ctx) {
  ...
  if(visStage.stage>prevStage&&visStage.stage>=2)c.narr('system','【'+visStage.name+'】'+visStage.description,{isSpecial:true});
...
export function _processRestRecovery(s, c, shStage, ctx) {
  ...
  }else{c.narr('system','没有食物，你无法从休息中恢复。',{isSpecial:true});}
...
export function _processDayOpenAndEndings(s, c, _startSan, _startHp, _startClues, _startArea, ctx) {
  ...
  if(area){var dayDesc=area.description;if(area.layout_variants&&area.layout_variants.length>0){var phase=getPhase(s.ap,s.maxAp);var isNight=phase==='midnight'||phase==='evening';var isRainy=s.weather==='雨天'||s.weather==='大雾';var visitCount=(s.visitedAreas||[]).filter(a=>a===s.currentArea).length;var eligible=area.layout_variants.filter(function(v){if(v.id.endsWith('_dark')&&!isNight)return false;if(v.id.endsWith('_flooded')&&!isRainy)return false;if(v.id.endsWith('_wrecked')&&visitCount<2)return false;return true;});if(eligible.length>0){var totalW=eligible.reduce(function(t,v){return t+(v.weight||1);},0);var r=Math.random()*totalW;var chosen=eligible[0];for(var _vi=0;_vi<eligible.length;_vi++){r-=(eligible[_vi].weight||1);if(r<=0){chosen=eligible[_vi];break;}}if(chosen.description)dayDesc+='\n\n'+chosen.description;}}c.narr('location',dayDesc,{locationName:getAreaDisplayName(area,s),imageSrc:getAreaSceneImage(s.currentArea,{...c.view,visits:(s.visitedAreas||[]).filter(a=>a===s.currentArea).length}),imageAlt:getAreaDisplayName(area,s),_areaClass:'area-scene-'+s.currentArea});}
