...
function handleNpcAction(s, action, c) {
  ...
  case 'TALK_NPC':{
    ...
    if(corrVariant!=='normal'){
      ...
      if(corrLines){
        ...
        if(lines&&lines.length>0&&Math.random()<0.4)c.narr('system',npc.name+': "'+lines[Math.floor(Math.random()*lines.length)]+'"');
      }
    }}
    ...
    if(ns.corrupted){
      ...
      if(corrLoss>0){s.san=clamp(s.san-corrLoss,0,s.maxSan);c.narr('system',npc.name+'的状态不对劲。SAN -'+corrLoss);}
      ...
      if(sanRec&&sanRec.normal_chat){
        if(sanRec.normal_chat.includes('SAN+1')){s.san=clamp(s.san+1,0,s.maxSan);c.narr('san-recovery',sanRec.description||('与'+npc.name+'交谈让你感到安慰。SAN +1'));}
        else{c.narr('system',sanRec.description||sanRec.normal_chat);}
      }else if(trust<3){
        const rec=d3()-1;if(rec>0){s.san=clamp(s.san+rec,0,s.maxSan);c.narr('san-recovery','与'+npc.name+'交谈让你感到些许安慰。SAN +'+rec);}
      }
    }
  ...
  case 'NPC_RESPONSE':{
    ...
    if(choice==='trust_up'){
      ...
      c.narr('system',silenceEntries.length>0?pick(silenceEntries):'你没有回答。沉默也是一种回答。');
      ...
      if(success){
        ...
        c.narr('system','【攻击】掷骰 '+roll+' / 格斗'+fightSkill+' —— 成功！'+npc.name+'倒下了。SAN -'+sanLoss,{isSpecial:true});
      ...
      if(s.ap>=1){s.ap-=1;c.narr('system','你花了一些时间处理现场。痕迹被抹去了。');}
      else{c.narr('system','你没有时间仔细处理，但你尽力隐藏了能隐藏的一切。');}
      ...
      c.narr('system','你做了无法挽回的事。食物+2。某种东西在你体内扎了根。SAN -'+sanLoss,{isSpecial:true});
      s.pendingNpc=null;
      ...
      c.narr('system','你利用了'+npc.name+'的信任。金钱 +'+gain+'。对方的眼神里多了一丝怀疑。');
      s.pendingNpc=null;
      ...
      c.narr('system','你把'+npc.name+'的秘密告诉了不该告诉的人。信任归零。你得到了一些东西——但不是钱。',{isSpecial:true});
      s.pendingNpc=null;
