...
async function initGame() {
  ...
  const ch2plus = await fetch('/game_ch2plus.json').then(r => r.json()).catch(() => ({}));
  const meta = await fetch('/game_meta.json').then(r => r.json()).catch(() => ({}));
  ...
  if (meta.deprecated_endings_archive) GD.deprecated_endings_archive = meta.deprecated_endings_archive;
  ...
  root.render(
    React.createElement('div', { style: { padding: '2rem', color: '#f0e6d3', fontFamily: 'serif' } },
