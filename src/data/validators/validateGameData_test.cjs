...
function validateGameData(base, ch2plus, meta) {
  ...
  const error = (rule, msg, ctx) =>
    errors.push({ level: 'error', rule, message: msg, context: ctx });
  ...
  (function () {
    ...
    for (const evt of base.events || []) {
      if (!evt.id) {
        error('E01', 'Event missing id in base', JSON.stringify(evt).slice(0, 80));
        continue;
      }
      if (baseIds.has(evt.id)) {
        error('E01', 'Duplicate event id in base: ' + evt.id);
      }
      baseIds.add(evt.id);
      eventIds.add(evt.id);
    }
    for (const evt of (ch2plus || {}).events || []) {
      if (!evt.id) {
        error('E01', 'Event missing id in ch2plus', JSON.stringify(evt).slice(0, 80));
        continue;
      }
      if (baseIds.has(evt.id)) {
        warn('E01', 'Event id overlaps base: ' + evt.id + ' (expected in split JSON)');
      }
      eventIds.add(evt.id);
    }
  })();
  ...
  (function () {
    ...
    for (const end of base.endings || []) {
      if (!end.id) {
        error('E02', 'Ending missing id in base', JSON.stringify(end).slice(0, 80));
        continue;
      }
      if (baseIds.has(end.id)) {
        error('E02', 'Duplicate ending id in base: ' + end.id);
      }
      baseIds.add(end.id);
    ...
    for (const end of (ch2plus || {}).endings || []) {
      if (!end.id) {
        error('E02', 'Ending missing id in ch2plus', JSON.stringify(end).slice(0, 80));
        continue;
      }
      if (baseIds.has(end.id)) {
        warn('E02', 'Ending id overlaps base: ' + end.id + ' (expected in split JSON)');
      }
      endingIds.add(end.id);
    }
  })();
  ...
  for (const end of endings) {
    ...
    for (const cond of conditions) {
      if (typeof cond !== 'string') {
        error('E07', 'Non-string condition in ' + end.id, end.id);
        continue;
      }
      ...
      for (const ch of cond) {
        ...
        if (ch === ')') depth--;
        if (depth < 0) {
          error('E07', 'Unbalanced parens in ' + end.id + ': ' + cond, end.id);
          break;
        }
      }
  ...
  if (e11Count > BASELINES.E11_chinese_npc_refs) {
    error(
      ...
      'Chinese NPC refs ' +
        e11Count +
        ' exceeds baseline ' +
        BASELINES.E11_chinese_npc_refs +
        '. New references must use stable IDs.'
    );
  } else if (e11Count > 0) {
    warn(
      'E11',
      'Chinese NPC refs: ' +
        ...
        '/' +
        BASELINES.E11_chinese_npc_refs +
        ' (within baseline, migrate when ready)'
    );
  }
  ...
  if (e12Count > BASELINES.E12_chinese_item_refs) {
    error(
      ...
      'Chinese item refs ' +
        e12Count +
        ' exceeds baseline ' +
        BASELINES.E12_chinese_item_refs +
        '. New references must use stable IDs.'
    );
  } else if (e12Count > 0) {
    warn(
  ...
  if (e13Count > BASELINES.E13_chinese_npc_conds) {
    error(
      'E13_BASELINE',
      'Chinese NPC ending conds ' +
        e13Count +
        ' exceeds baseline ' +
        BASELINES.E13_chinese_npc_conds
    );
  } else if (e13Count > 0) {
    warn(
      'E13',
    ...
    for (const c of nc) {
      ...
      if (name && !npcNameList.includes(name)) {
        ...
        if (/[一-鿿]/.test(name)) {
          error(
            'E14',
            'Unknown Chinese NPC reference in ' + evt.id + ': ' + name + ' (not in npcRegistry)',
            evt.id
          );
        }
      }
  ...
  try {
    itemRegistry = require('./itemRegistry.js');
  } catch (e) {}
  if (!itemRegistry)
    try {
      itemRegistry = require('../registry/itemRegistry.cjs');
    } catch (e) {}

  if (itemRegistry && itemRegistry.ITEM_REGISTRY) {
    var knownItemNames = new Set(
      Object.values(itemRegistry.ITEM_REGISTRY).map(function (e) {
        return e.name;
      })
    );
    ...
    for (var id in itemRegistry.ITEM_REGISTRY) {
      ...
      for (var ai = 0; ai < (entry.aliases || []).length; ai++) {
        ...
        if (aliasMap[alias])
          error(
            'E16',
            'Duplicate item alias: "' + alias + '" in ' + id + ' and ' + aliasMap[alias]
          );
        aliasMap[alias] = id;
      }
    }
