...
function validateReferences(base, ch2plus, meta) {
  ...
  var areas = new Set((base.areas||[]).map(function(a){return a.id;}));
  var npcNames = new Set((base.npcs||[]).map(function(n){return n.name;}));
  ...
  (function(){
    ...
    (base.events||[]).forEach(function(e){
      if(!e.id){error('E01','Event missing id');return;}
      if(baseIds.has(e.id))error('E01','Dup event in base: '+e.id);
      baseIds.add(e.id);eventIds.add(e.id);
    });
  ...
  (function(){
    ...
    (base.endings||[]).forEach(function(e){
      if(!e.id){error('E02','Ending missing id');return;}
      if(baseIds.has(e.id))error('E02','Dup ending in base: '+e.id);
      baseIds.add(e.id);endingIds.add(e.id);
    });
  ...
  events.forEach(function(evt){
    ...
    arr.forEach(function(a){
      if(a&&a!=='any'&&a!=='all'&&!areas.has(a))error('E04','Unknown area in '+evt.id+': '+a);
    });
  });
  ...
  (ej.priority_order||[]).forEach(function(eid){
    if(!endingIds.has(eid)){
      ...
      var inArchive=Array.isArray(archive)&&archive.some(function(e){return e.id===eid;});
  ...
  (base.event_chains||[]).forEach(function(chain){
    (chain.sequence||[]).forEach(function(eid){
      if(!eventIds.has(eid))warn('E10','Chain ref unknown: '+(chain.name||'?')+' -> '+eid);
    });
  });
...
try{module.exports={validateReferences};}catch(e){}
