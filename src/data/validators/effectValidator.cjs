...
function validateEffects(base, ch2plus) {
  ...
  var npcs = base.npcs || [];
  var npcNames = npcs.map(function (n) {
    return n.name;
  });
  ...
  try {
    var b = require('./baselines.json');
    Object.assign(BASELINES, b);
  } catch (e) {}
  ...
  if (e11Count > BASELINES.chineseNpcRefs)
    error(
      'E11_BASELINE',
      'Chinese NPC refs ' + e11Count + ' exceeds baseline ' + BASELINES.chineseNpcRefs
    );
  else if (e11Count > 0)
    warn('E11', 'Chinese NPC refs: ' + e11Count + '/' + BASELINES.chineseNpcRefs);
  ...
  if (e12Count > BASELINES.chineseItemRefs)
    error(
      'E12_BASELINE',
      'Chinese item refs ' + e12Count + ' exceeds baseline ' + BASELINES.chineseItemRefs
    );
  else if (e12Count > 0)
    warn('E12', 'Chinese item refs: ' + e12Count + '/' + BASELINES.chineseItemRefs);

  // E13: Chinese NPC in ending conditions (baseline ratchet)
  var e13Count = 0;
  endings.forEach(function (end) {
    var allConds = []
      .concat(end.required_conditions || [])
      .concat(end.blocking_conditions || [])
      .concat([JSON.stringify(end.npc_requirements || {})])
      .join(' ');
    npcNames.forEach(function (name) {
      if (allConds.indexOf(name) >= 0) e13Count++;
    });
  });
  if (e13Count > BASELINES.chineseNpcConditions)
    error(
      'E13_BASELINE',
      'Chinese NPC ending conds ' + e13Count + ' exceeds baseline ' + BASELINES.chineseNpcConditions
    );
  else if (e13Count > 0)
    warn('E13', 'Chinese NPC ending conds: ' + e13Count + '/' + BASELINES.chineseNpcConditions);

  // E14: Unknown Chinese NPC references
  events.forEach(function (evt) {
    (evt.effects?.npc_changes || []).forEach(function (c) {
      var name = c.name || c.npc;
      if (name && npcNames.indexOf(name) < 0 && /[一-鿿]/.test(name))
        error('E14', 'Unknown Chinese NPC in ' + evt.id + ': ' + name);
    });
  });
  ...
  try {
    itemRegistry = require('./itemRegistry.cjs');
  } catch (e) {}
  try {
    if (!itemRegistry) itemRegistry = require('../registry/itemRegistry.cjs');
  } catch (e) {}
  if (itemRegistry && itemRegistry.ITEM_REGISTRY) {
    ...
    events.forEach(function (evt) {
      (evt.effects?.items || []).forEach(function (item) {
        if (typeof item === 'string') {
          ...
          if (resolved === item && /[一-鿿]/.test(item) && !knownNames.has(item))
            error('E15', 'Unknown item in ' + evt.id + ': ' + item);
        }
      });
    });
    ...
    for (var aid in itemRegistry.ITEM_REGISTRY) {
      (itemRegistry.ITEM_REGISTRY[aid].aliases || []).forEach(function (alias) {
        if (aliasMap[alias])
          error('E16', 'Dup alias: "' + alias + '" in ' + aid + ' and ' + aliasMap[alias]);
        aliasMap[alias] = aid;
      });
    }
    // E17: id naming
...
try {
  module.exports = { validateEffects };
} catch (e) {}
