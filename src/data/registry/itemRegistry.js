...
export var CLUE_ITEMS = [
  ...
  '银质护身符','暗河水样','河底碎片（未知材质）','壁画临摹（含隐藏通道线索）',
  '食尸鬼秘物','伊斯记忆碎片（可学习）','时间碎片','透镜碎片','腐蚀之血','古树种子',
  '修格斯残留物（未知用途）','封印仪式记录（关键线索）',
  '虚空回响记忆（永久效果：克苏鲁神话+3）','维度碎片','以诺·沃特的日记',
  '海底神殿草图','深海之水','深潜者祭品碎片','克苏鲁之梦记忆','深渊水晶',
  '祭祀记忆碎片','奈亚拉托提普印记','骨之花园记录','石碑拓片碎片',
  '伊斯人残影记忆','机器示意图残片','星图石板','莫里斯家族封印记录',
  '守护符文','可疑商品目录',
];
CLUE_ITEMS.forEach(function(name,i){
  var id='clue_item_'+i;
  if(!ITEM_REGISTRY[id]) ITEM_REGISTRY[id]={name:name,aliases:[],type:'clue',stackable:false};
});
...
try{module.exports={ITEM_REGISTRY,ITEM_NAME_TO_ID,resolveItemId,getItemName,normalizeItemRef,migrateInventory,addItem,removeItem,hasItem,getItemCount};}catch(e){}
