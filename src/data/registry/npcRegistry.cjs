...
var NPC_NAME_TO_ID = _h ? _h.nameToId : (function(){var m={};for(var id in NPC_REGISTRY){var e=NPC_REGISTRY[id];m[e.name]=id;(e.aliases||[]).forEach(function(a){m[a]=id;});}return m;})();

function resolveNpcId(input) { return _h ? _h.resolveId(input) : (NPC_REGISTRY[input] ? input : (NPC_NAME_TO_ID[input] || input)); }
function getNpcName(input)   { return _h ? _h.getName(input) : ((NPC_REGISTRY[input] && NPC_REGISTRY[input].name) || input); }
function migrateNpcKeys(obj) { return _h ? _h.migrateKeys(obj) : obj; }
