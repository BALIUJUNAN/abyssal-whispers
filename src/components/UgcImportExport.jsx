...
import { getAllMods, installMod, uninstallMod, toggleMod, exportMod, importModFromJson, getModStats } from '../reducers/ugcReducer.js';
...
export function UgcPanel({ onClose, GD }) {
  ...
  return <div className="ugc-panel">
    <div className="ugc-panel-header">
      ...
      <button className="ugc-close-btn" onClick={onClose}>✕</button>
    </div>

    {feedback && <div className={'ugc-feedback ugc-feedback-' + feedback.type}>{feedback.msg}</div>}

    <div className="ugc-panel-body">
      {view === 'list' && <ModListView
        mods={mods}
        onToggle={handleToggle}
        onUninstall={handleUninstall}
        onExport={handleExport}
        onSelect={(m) => { setSelectedMod(m); setView('detail'); }}
        onImport={() => setView('import')}
      />}
      {view === 'import' && <ModImportView
        GD={GD}
        onBack={() => setView('list')}
        onSuccess={handleImportSuccess}
        onError={(msg) => showToast(msg, 'error')}
      />}
      {view === 'detail' && selectedMod && <ModDetailView
        mod={selectedMod}
        onBack={() => { setView('list'); setSelectedMod(null); }}
        onToggle={() => handleToggle(selectedMod.id)}
        onExport={() => handleExport(selectedMod.id)}
        onUninstall={() => { handleUninstall(selectedMod.id); }}
      />}
    </div>

    {/* Sample mod download */}
    {view === 'list' && mods.length === 0 && <div className="ugc-empty-state">
      ...
      <button className="btn btn-sm" onClick={() => downloadSampleMod()}>下载示例模组</button>
    </div>}
  </div>;
}
...
function ModListView({ mods, onToggle, onUninstall, onExport, onSelect, onImport }) {
  return <>
    <div className="ugc-toolbar">
      <button className="btn btn-primary btn-sm" onClick={onImport}>📥 导入模组</button>
      <button className="btn btn-sm" onClick={() => downloadSampleMod()}>📄 示例模组</button>
    </div>
    {mods.length > 0 && <div className="ugc-mod-list">
      {mods.map(mod => <div key={mod.id} className={'ugc-mod-card' + (mod.enabled === false ? ' disabled' : '')}>
        ...
        <div className="ugc-mod-card-actions">
          <button className="btn btn-sm" onClick={() => onSelect(mod)}>详情</button>
          <button className="btn btn-sm" onClick={() => onExport(mod.id)}>导出</button>
          <button className="btn btn-sm btn-danger" onClick={() => onUninstall(mod.id)}>卸载</button>
        </div>
      </div>)}
    </div>}
  </>;
  ...
  return <div className="ugc-import-view">
    ...
    >
      ...
      <textarea
        ...
        value={jsonText}
        onChange={(e) => { setJsonText(e.target.value); setValidation(null); }}
    ...
    <div className="ugc-import-actions">
      <button className="btn" onClick={handleValidate} disabled={!jsonText.trim()}>🔍 验证</button>
      <button className="btn btn-primary" onClick={handleInstall} disabled={!validation?.valid || installing}>
        {installing ? '安装中...' : '📦 安装模组'}
      </button>
    </div>
...
function ModDetailView({ mod, onBack, onToggle, onExport, onUninstall }) {
  ...
  return <div className="ugc-detail-view">
    ...
    <div className="ugc-detail-meta">
      <div><strong>作者：</strong>{mod.author}</div>
      <div><strong>版本：</strong>{mod.version}</div>
      <div><strong>事件数：</strong>{mod.events?.length || 0}</div>
      <div><strong>状态：</strong>{mod.enabled !== false ? '✅ 已启用' : '⏸ 已禁用'}</div>
      {mod.createdAt && <div><strong>创建：</strong>{new Date(mod.createdAt).toLocaleDateString()}</div>}
    </div>

    <div className="ugc-detail-events">
      <h4>事件列表</h4>
      {(mod.events || []).map((evt, i) => <div key={evt.id} className="ugc-event-card" onClick={() => setExpandedEvent(expandedEvent === i ? null : i)}>
        ...
        {expandedEvent === i && <div className="ugc-event-detail">
          <div className="ugc-event-desc">{evt.description}</div>
          {evt.choices && evt.choices.length > 0 && <div className="ugc-event-choices">
            <strong>选项：</strong>
            {evt.choices.map(c => <div key={c.id} className="ugc-event-choice">· {c.label}</div>)}
          </div>}
          {evt.trigger && <div className="ugc-event-trigger">
            <strong>触发条件：</strong> {JSON.stringify(evt.trigger)}
          </div>}
          {evt.effects && <div className="ugc-event-effects">
    ...
    <div className="ugc-detail-actions">
      <button className={'btn' + (mod.enabled !== false ? '' : ' btn-primary')} onClick={onToggle}>
        {mod.enabled !== false ? '⏸ 禁用' : '▶ 启用'}
      </button>
      <button className="btn" onClick={onExport}>📥 导出</button>
      <button className="btn btn-danger" onClick={onUninstall}>🗑 卸载</button>
    </div>
  </div>;
}
...
function ValidationResult({ result }) {
  if (result.valid) {
    ...
    return <div className="ugc-validation ugc-validation-pass">
      <div className="ugc-validation-title">✅ 验证通过</div>
      <div className="ugc-validation-info">
        <div>模组：{result.sanitized?.name} v{result.sanitized?.version}</div>
        <div>作者：{result.sanitized?.author}</div>
        <div>事件数：{evtCount}</div>
      </div>
      {result.warnings?.length > 0 && <div className="ugc-validation-warnings">
        {result.warnings.map((w, i) => <div key={i}>⚠️ {w}</div>)}
      </div>}
    </div>;
  }

  return <div className="ugc-validation ugc-validation-fail">
    <div className="ugc-validation-title">❌ 验证失败</div>
    <div className="ugc-validation-errors">
      {result.errors.slice(0, 10).map((e, i) => <div key={i} className="ugc-error-item">· {e}</div>)}
      {result.errors.length > 10 && <div className="ugc-error-more">...还有 {result.errors.length - 10} 个错误</div>}
    </div>
  </div>;
}
