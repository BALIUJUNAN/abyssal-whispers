...
export function NPCDialog({ npc, trust, layer, dispatch, state }) {
  ...
  if (postKill) {
    return (
      <div className="narrative-block npc-dialogue">
        <div className="skill-check" style={{ borderLeft: '2px solid var(--danger)' }}>
          <div style={{ color: 'var(--danger2)', fontSize: '0.9rem', marginBottom: '0.3rem' }}>
            {npc.name} 已死
          </div>
          <div
            style={{
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.3rem' }}>
            <button
              ...
              onClick={() => dispatch({ type: 'NPC_RESPONSE', choice: 'post_kill_hide' })}
            >
              隐藏尸体<span className="cost">1 AP 痕迹+1</span>
            </button>
            <button
              className="btn btn-sm"
              style={{ color: 'var(--danger2)' }}
              onClick={() => dispatch({ type: 'NPC_RESPONSE', choice: 'post_kill_cannibal' })}
            >
              食用<span className="cost">食物+2 SAN大幅下降</span>
            </button>
            <button
              className="btn btn-sm"
              onClick={() => dispatch({ type: 'NPC_RESPONSE', choice: 'post_kill_leave' })}
            >
              离开现场
            </button>
          </div>
  ...
  const doResponse = (choice) => {
    ...
    if (delay > 0) {
      setTimeout(() => {
        dispatch({ type: 'NPC_RESPONSE', choice });
        setShow(false);
      }, delay);
    } else {
      dispatch({ type: 'NPC_RESPONSE', choice });
  ...
  return (
    <div className="narrative-block npc-dialogue">
      <div className="skill-check">
        ...
        >
          ...
          {Array.from({ length: 5 }, (_, i) => (
            <span
              key={i}
              style={{
                color: i < trust ? 'var(--gold)' : 'var(--border2)',
                textShadow: i < trust ? '0 0 4px rgba(184,150,58,0.3)' : 'none',
              }}
            >
              ★
            </span>
        ...
        {ns.corrupted && (
          <div style={{ fontSize: '0.7rem', color: 'var(--danger2)', marginBottom: '0.3rem' }}>
            ⚠ 该NPC已被腐蚀
          </div>
        )}
        {ns.fled && (
        ...
        {!show ? (
          ...
          <div>
            {/* 确认操作（攻击/背叛） */}
            {confirmAction && (
              <div className="npc-confirm">
                <div
                  style={{ color: 'var(--danger2)', fontSize: '0.8rem', marginBottom: '0.4rem' }}
                >
                  ⚠ 确定要{confirmAction === 'attack' ? '攻击' : '背叛'}吗？此操作不可撤销。
                </div>
                <div style={{ display: 'flex', gap: '0.4rem' }}>
                  <button
                    className="btn btn-sm btn-danger"
                    onClick={() => {
                      dispatch({ type: 'NPC_RESPONSE', choice: confirmAction });
                      setShow(false);
            ...
            <div className="npc-dialog-group">
              ...
              {!collapsedGroups.talk && (
                <div className="npc-dialog-group-body">
                  {trust < 5 &&
                    (() => {
                      const already = state?._dailyTrustGains?.[npc.name];
                      const gate = checkTrustGate(trust + 1, state, npc.name);
                      if (already)
                        return (
                          <div
                            style={{
                      ...
                      return (
                        <button className="btn btn-sm" onClick={() => doResponse('trust_up')}>
                          {trust < 2 ? '尝试建立信任' : '加深了解'}
                          <span className="cost">1 AP 信任+1</span>
                        </button>
                      );
                    })()}
                  <button className="btn btn-sm" onClick={() => doResponse('silence')}>
                    保持沉默
                  </button>
                </div>
              )}
            ...
            <div className="npc-dialog-group">
              ...
              {!collapsedGroups.help && (
                <div className="npc-dialog-group-body">
                  {trust >= 1 && npc.secrets && trust <= npc.secrets.length && (
                    <button className="btn btn-sm" onClick={() => doResponse('get_item')}>
                      询问更多信息
                    </button>
                  )}
                  {(() => {
                    const already = state?._dailyTrustGains?.[npc.name];
                    const curTrust = state?.npcTrust?.[npc.name] || 0;
                    const gate = trust < 5 ? checkTrustGate(curTrust + 1, state, npc.name) : 'max';
                    if (already) return null;
                    if (trust >= 5) return null;
                    return (
                      <button
                        className="btn btn-sm"
                        onClick={() => doResponse('share_food')}
                        disabled={(state?.food || 0) < 1 || (state?.ap || 0) < 1 || !!gate}
                        title={gate || ''}
                      >
                        分享食物
                        {gate ? (
                          <span className="cost">🔒 {gate.slice(0, 15)}…</span>
                        ) : (
                          <span className="cost">1 AP 食物-1 信任+1</span>
                        )}
                      </button>
                    );
                  })()}
                  {ns.corrupted && trust >= 4 && (
                    <button
                      className="btn btn-sm"
                      style={{ color: 'var(--gold)' }}
                      onClick={() => doResponse('redeem')}
                    >
                      尝试救赎
                    </button>
                  )}
            ...
            {/* 特殊组（默认折叠） */}
            {hasSpecial && (
              <div className="npc-dialog-group npc-dialog-group--danger">
                <div className="npc-dialog-group-title" onClick={() => toggleGroup('special')}>
                  <span className={'chevron' + (collapsedGroups.special ? '' : ' open')}>▶</span>{' '}
                  特殊
                </div>
                {!collapsedGroups.special && (
                  <div className="npc-dialog-group-body">
                    ...
                    {trust >= 2 && (
                      <button
                        className="btn btn-sm"
                        style={{ color: 'var(--danger2)', fontSize: '0.72rem' }}
                        onClick={() => doResponse('incite')}
                      >
                        ⚠ 陷害（2 AP 话术检定）
                      </button>
                    )}
                    {trust >= 2 && (
                      <button
                        className="btn btn-sm"
                        style={{ color: 'var(--text-dim)', fontSize: '0.72rem' }}
                        onClick={() => doResponse('exploit_npc')}
                      >
                        ⚠ 利用（1 AP 信任-2）
                      </button>
                    )}
                    {trust >= 3 && (
                      <button
                        className="btn btn-sm"
                        style={{ color: 'var(--danger2)', fontSize: '0.72rem' }}
                        onClick={() => setConfirmAction('betray_npc')}
                      >
                        ⚠ 背叛（1 AP 信任清零）
                      </button>
                    )}
                    {ns.corrupted && trust >= 2 && (
                      <button
                        className="btn btn-sm"
                        style={{ color: 'var(--danger2)', fontSize: '0.72rem' }}
                        onClick={() => doResponse('intimacy')}
                      >
                        ⚠ 靠近（2 AP SAN-）
                      </button>
                    )}
                    <button
                      ...
                      onClick={() => setConfirmAction('attack')}
                    >
                      ⚠ 攻击（2 AP 格斗检定）
                    </button>
                  </div>
            ...
            <div
              style={{
                marginTop: '0.35rem',
                borderTop: '1px solid rgba(42,42,48,0.2)',
                paddingTop: '0.3rem',
              }}
            >
              <button className="btn btn-sm" onClick={() => doResponse('leave')}>
                告别
              </button>
            </div>
          </div>
