  ...
  return (
    <div className={'stat-bar ' + (cls || '')}>
      ...
      <div className="bar-track">
        <div
          className="bar-fill"
          style={bg ? { width: pct + '%', background: bg } : { width: pct + '%' }}
        />
      </div>
    </div>
  );
...
export function CollapsibleSection({ title, count, defaultOpen, summary, children }) {
  ...
  return (
    <div className="dossier-section">
      ...
      <div
        className={'dossier-section-body' + (open ? ' expanded' : ' collapsed')}
        style={open ? { maxHeight: '600px' } : undefined}
      >
        {children}
      </div>
    </div>
...
export const NarrativeBlock = memo(function NarrativeBlock({ block }) {
  ...
  const eventTypeLabel = block.eventType
    ? EVENT_TYPE_LABELS[block.eventType] || block.eventType
    : null;
  return (
    <div
      className={
        'narrative-block' +
        (block.type === 'system' ? ' system' : '') +
        (block.isEffect ? ' system' : '') +
        (block.isSpecial ? ' system' : '') +
        (block.type === 'death' ? ' death-narrative' : '') +
        (isSanRecovery ? ' san-recovery' : '') +
        (isMythos ? ' mythos-text' : '') +
        (block._areaClass ? ' ' + block._areaClass : '')
      }
    >
      ...
      {block.eventTitle && (
        <div className="event-title">
          {block._ugcAuthor ? (
            <span className="ugc-badge" title={'MOD by ' + block._ugcAuthor}>
              🏷️ [MOD]
            </span>
          ) : null}
          {block.eventTitle}
      ...
      {block.madness && (
        <div className="madness-effect">
          ⚠ {block.madness.name}：{block.madness.description}
        </div>
      )}
    </div>
  );
