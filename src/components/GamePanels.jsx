...
// LeftPanel, CenterPanel, NPCDialog, RightPanel, CitySketchMap, EndingScreen, GameHeader
    ...
    <div className={'narrative-area'+percCls} ref={ref}>
      ...
      {state.pendingGamble&&<div className="narrative-block"><div className="skill-check">
        ...
        <div style={{display:'flex',gap:'0.5rem',flexWrap:'wrap'}}>
          {state.pendingGamble.options.map(opt=>{
            ...
            return <CorruptibleChoice key={opt.id} className={'btn btn-sm'+(opt.id==='deep_investigate'?' btn-danger':'')} san={state.san} onClick={()=>dispatch({type:'GAMBLE_CHOICE',choiceId:opt.id})}>{label}<span className="cost">{risk}</span></CorruptibleChoice>;
          })}
        </div>
      </div></div>}
      {state.pendingChoice&&<div className="narrative-block"><div className="skill-check">
        ...
        <div style={{display:'flex',flexDirection:'column',gap:'0.3rem',marginTop:'0.3rem'}}>
          {state.pendingChoice.choices.map((ch,i)=><CorruptibleChoice key={i} className="btn btn-sm" san={state.san} onClick={()=>dispatch({type:'CHOICE_SELECT',choiceIdx:i})}>{ch.label}</CorruptibleChoice>)}
        </div>
      </div></div>}
    </div>
