...
export var CorruptibleChoice = memo(function (props) {
  ...
  var V = getVisualForSan(san, { GD: GD }); // This is called once per render, not in animation loop — OK
...
export function AbyssPopup(props) {
  ...
  useEffect(
    function () {
      var _slvl = getVisualForSan(san, { GD: GD }).level || 0; // Called once per effect, not in animation loop — OK
      if (_slvl < 3) {
        setVisible(false);
        return;
      }
